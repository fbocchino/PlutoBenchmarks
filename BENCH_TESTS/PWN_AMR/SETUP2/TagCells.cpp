...
#if STATIC_REFINEMENT == YES           
    grad[k][j][i] = 1.0;
#endif

    if      ( r <= 0.1 )                grad[k][j][i] = 1.0;
    else if ( r <= 0.5 )    						grad[k][j][i] *= (m_level <= 5 ? 1.0:0.0);
    else if ( r <= 1.0 )     						grad[k][j][i] *= (m_level <= 4 ? 1.0:0.0);
    else if ( r <= 2.0 )     						grad[k][j][i] *= (m_level <= 3 ? 1.0:0.0);
    else if ( r <= 3.0 )      					grad[k][j][i] *= (m_level <= 2 ? 1.0:0.0);
    else if ( r <= 5.0 )      					grad[k][j][i] *= (m_level <= 1 ? 1.0:0.0);
    else if ( r > 5.0 )                 grad[k][j][i] *= (m_level <= 0 ? 1.0:0.0);
    else                     						grad[k][j][i] = 0.0;
